...
# + authProvider - ClientOAuth2Provider to generate the access token
public client class Client {
    ...
    public isolated function init(SalesforceConfiguration salesforceConfig) returns error? {
        ...
        self.authProvider = new (salesforceConfig.clientConfig);
    ...
    isolated remote function describeAvailableObjects() returns @tainted OrgMetadata|Error {
    ...
    isolated remote function getSObjectBasicInfo(string sobjectName) returns @tainted SObjectBasicInfo|Error {
    ...
    isolated remote function describeSObject(string sObjectName) returns @tainted SObjectMetaData|Error {
    ...
    # + return - `SObjectBasicInfo` record if successful else Error occured
    isolated remote function sObjectPlatformAction() returns @tainted SObjectBasicInfo|Error {
    ...
    isolated remote function getRecord(string path) returns @tainted json|Error {
    ...
    isolated remote function createRecord(string sObjectName, json recordPayload) returns @tainted string|Error {
    ...
    isolated remote function updateRecord(string sObjectName, string id, json recordPayload) returns @tainted boolean|Error {
        http:Request req = new;
    ...
    # + return - `json` result if successful else `Error` occured
    isolated remote function getRecordById(string sobject, string id, string... fields) returns @tainted json|Error {
    ...
    # + fields - fields to retrieve 
    # + return - `json` result if successful else `Error` occured
    isolated remote function getRecordByExtId(string sobject, string extIdField, string extId, string... fields) returns @tainted json|
    Error {
    ...
    isolated remote function getAccountById(string accountId, string... fields) returns @tainted json|Error {
    ...
    isolated remote function createAccount(json accountRecord) returns @tainted string|Error {
    ...
    isolated remote function deleteAccount(string accountId) returns @tainted boolean|Error {
    ...
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function updateAccount(string accountId, json accountRecord) returns @tainted boolean|Error {
    ...
    # + fields - Fields to retireve
    # + return - JSON response if successful or else an sfdc:Error
    isolated remote function getLeadById(string leadId, string... fields) returns @tainted json|Error {
    ...
    isolated remote function createLead(json leadRecord) returns @tainted string|Error {
    ...
    isolated remote function deleteLead(string leadId) returns @tainted boolean|Error {
    ...
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function updateLead(string leadId, json leadRecord) returns @tainted boolean|Error {
    ...
    isolated remote function getContactById(string contactId, string... fields) returns @tainted json|Error {
    ...
    isolated remote function createContact(json contactRecord) returns @tainted string|Error {
    ...
    # + contactId - Contact ID
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function deleteContact(string contactId) returns @tainted boolean|Error {
    ...
    # + contactRecord - JSON contact record
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function updateContact(string contactId, json contactRecord) returns @tainted boolean|Error {
    ...
    # + fields - Fields to retireve
    # + return - JSON response if successful or else an sfdc:Error
    isolated remote function getOpportunityById(string opportunityId, string... fields) returns @tainted json|Error {
    ...
    isolated remote function createOpportunity(json opportunityRecord) returns @tainted string|Error {
    ...
    # + opportunityId - Opportunity ID
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function deleteOpportunity(string opportunityId) returns @tainted boolean|Error {
    ...
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function updateOpportunity(string opportunityId, json opportunityRecord) returns @tainted boolean|Error {
    ...
    # + fields - Fields to retireve
    # + return - JSON result if successful or else an sfdc:Error
    isolated remote function getProductById(string productId, string... fields) returns @tainted json|Error {
    ...
    isolated remote function createProduct(json productRecord) returns @tainted string|Error {
    ...
    # + productId - Product ID
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function deleteProduct(string productId) returns @tainted boolean|Error {
    ...
    # + productRecord - JSON product record
    # + return - `true` if successful, `false` otherwise or an sfdc:Error in case of an error
    isolated remote function updateProduct(string productId, json productRecord) returns @tainted boolean|Error {
    ...
    isolated remote function getQueryResult(string receivedQuery) returns @tainted SoqlResult|Error {
    ...
    # + return - `SoqlResult` record if successful. Else, the occurred `Error`.
    isolated remote function getNextQueryResult(string nextRecordsUrl) returns @tainted SoqlResult|Error {
    ...
    isolated remote function searchSOSLString(string searchString) returns @tainted SoslResult|Error {
    ...
    isolated remote function getAvailableApiVersions() returns @tainted Version[]|Error {
    ...
    isolated remote function getResourcesByApiVersion(string apiVersion) returns @tainted map<string>|Error {
    ...
    isolated remote function getOrganizationLimits() returns @tainted map<Limit>|Error {
    ...
    isolated remote function creatJob(OPERATION operation, string sobj, JOBTYPE contentType, string extIdFieldName = "") returns @tainted error|
    BulkJob {
        json jobPayload = {
            "operation": operation,
            "object": sobj,
        ...
        var headerMap = getBulkAPIHeaders(self.authProvider);
    ...
    isolated remote function getJobInfo(BulkJob bulkJob) returns @tainted error|JobInfo {
    ...
    isolated remote function addBatch(BulkJob bulkJob, json|string|xml|io:ReadableByteChannel content) returns @tainted error|BatchInfo {
        ...
        if (bulkJob.jobDataType == JSON) {
            ...
            var headerMap = getBulkAPIHeaders(self.authProvider, APP_JSON);
            if (headerMap is map<string>) {
            ...
            var headerMap = getBulkAPIHeaders(self.authProvider, APP_XML);
            if (headerMap is map<string>) {
                var response = self.salesforceClient->post(path, payload, headers = headerMap);
            ...
            var headerMap = getBulkAPIHeaders(self.authProvider, TEXT_CSV);
            if (headerMap is map<string>) {
                var response = self.salesforceClient->post(path, payload, headers = headerMap);
                xml|Error batchResponse = checkXmlPayloadAndSetErrors(response);
                if (batchResponse is xml) {
    ...
    # + return - batch info or error
    isolated remote function getBatchInfo(BulkJob bulkJob, string batchId) returns @tainted error|BatchInfo {
    ...
    isolated remote function getAllBatches(BulkJob bulkJob) returns @tainted error|BatchInfo[] {
        string path = prepareUrl([SERVICES, ASYNC, BULK_API_VERSION, JOB, bulkJob.jobId, BATCH]);
        var headerMap = getBulkAPIHeaders(self.authProvider, TEXT_CSV);
        if (headerMap is map<string>) {
            var response = self.salesforceClient->get(path, headerMap);
    ...
    isolated remote function getBatchRequest(BulkJob bulkJob, string batchId) returns @tainted error|json|xml|string {
    ...
    # + batchId - batch ID
    # + return - result list
    isolated remote function getBatchResult(BulkJob bulkJob, string batchId) returns @tainted error|json|xml|string|Result[] {
...
public type SalesforceConfiguration record {
    string baseUrl;
    http:OAuth2RefreshTokenGrantConfig clientConfig;
